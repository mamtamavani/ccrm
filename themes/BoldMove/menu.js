...
popupMenu(id,menu);}
function tbButtonMouseOver(id,top,left,leftOffset,topOffset){closeMenusDelay=eraseTimeout(closeMenusDelay);if(openMenusDelay==null){openMenusDelay=window.setTimeout("showMenu('"+id+"','"+top+"','"+left+"','"+leftOffset+"','"+topOffset+"')",delayTime);}}
function showMenu(id,top,left,leftOffset,topOffset){openMenusDelay=eraseTimeout(openMenusDelay);var menuName=id.replace(/Handle/i,'Menu');var menu=getLayer(menuName);if(currentMenu){closeAllMenus();}
...
if(topOffset!='undefined'){top+=parseInt(topOffset);}}
