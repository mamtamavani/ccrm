 ...
 * "Powered by SugarCRM".
 */
SUGAR.themes.updateDelay=250;SUGAR.themes.updateSubTabHtml=function(mainTab){var subTabs=document.getElementById("subtabs");subTabs.innerHTML=SUGAR.themes.subTabs[mainTab];var menuHandle=document.getElementById(mainTab+"_sub_tab_id");var parentMenu=document.getElementById(mainTab+"_tab");var left='';if(left==""){p=parentMenu;var left=0;while(p&&p.tagName.toUpperCase()!='BODY'){left+=p.offsetLeft;p=p.offsetParent;}}
var bw=checkBrowserWidth();if(!parentMenu){return;}
var groupTabLeft=left+(parentMenu.offsetWidth / 2);var subTabHalfLength=0;var children=menuHandle.getElementsByTagName('li');for(var i=0;i<children.length;i++){subTabHalfLength+=parseInt(children[i].offsetWidth);}
...
if(left>=0){menuHandle.style.marginLeft=left+'px';}};SUGAR.themes.updateSubTabs=function(mainTab,htmlTab){if(typeof htmlTab!='undefined'){SUGAR.themes.updateSubTabHtml(htmlTab);}else{SUGAR.themes.updateSubTabHtml(mainTab);}
if(SUGAR.themes.activeTab){var oldActive=document.getElementById(SUGAR.themes.activeTab+"_tab");oldActive.getElementsByTagName('a')[0].className='otherTab';oldActive.className='otherTab';}
var newActive=document.getElementById(mainTab+"_tab");newActive.getElementsByTagName('a')[0].className='currentTab';newActive.className='currentTab';SUGAR.themes.activeTab=mainTab;};SUGAR.themes.updateMoreTab=function(tabName){var moreTab=document.getElementById(SUGAR.themes.moreTab+"_tab");if(moreTab==null)
return;if(SUGAR.themes.moreTabUrl==null){SUGAR.themes.moreTabUrl=moreTab.getElementsByTagName('a')[0].href;}
var href=(tabName!=SUGAR.themes.moreTab)?document.getElementById(tabName+"Handle").href:SUGAR.themes.moreTabUrl;moreTab.onmouseover=function(){SUGAR.themes.updateSubTabsDelay(SUGAR.themes.moreTab,tabName);};moreTab.getElementsByTagName('a')[0].innerHTML=tabName;moreTab.getElementsByTagName('a')[0].href=href;moreTab.getElementsByTagName('a')[0].onclick=function(){SUGAR.themes.chooseTab(tabName);return true;};};SUGAR.themes.resetTabs=function(){SUGAR.themes.updateSubTabs(SUGAR.themes.startTab);SUGAR.themes.updateMoreTab(SUGAR.themes.moreTab);}
SUGAR.themes.firstReset=function(){SUGAR.themes.resetTabs();}
