...
else
{
	$xtpl->assign("LBL_CHARSET", $default_charset);
								...
								}else if(file_exists('custom/' . $theme_path.'images/company_logo.png')){
...
$xtpl->assign("LEFT_FORM_OTD", '<img style="cursor: pointer; cursor: hand; margin-top: 10px;" id="HideHandle" name="HideHandle" src="'.$image_path.'show.gif" alt="" onclick=\'hideLeftCol("leftCol");closeMenus();\' onmouseover="showSubMenu(\'leftCol\')">');
...
$newform=function_exists('get_new_record_form') ? get_new_record_form() : "";
if ($currentModule && $action == "index" && $newform !="") {
	$xtpl->assign("NEW_RECORD", $newform);
