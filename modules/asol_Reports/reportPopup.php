...
echo "var postStr = 'exportedReportFile=".$_REQUEST['exportedReportFile']."&return_action=".$_REQUEST['return_action']."&pngs=".rawurlencode($_REQUEST['pngs'])."&legends=".rawurlencode($_REQUEST['legends'])."';";

if (in_array($_REQUEST['return_action'], array("ManualTasks", "SendToApp"))) {
	
	echo "postAjaxWithFunction('index.php?entryPoint=vRender', postStr, callBackClose);";
	
} else if ($_REQUEST['return_action'] != "downloadCSS") {

	echo "postAjaxWithFunction('index.php?entryPoint=vRender', postStr, callBackDownload);";
...
} else if (in_array($_REQUEST['return_action'], array("ExportHtml", "ExportPdf", "ExportCsv"))) {
...
} else if (in_array($_REQUEST['return_action'], array("SendToApp"))) {
	
	echo "<div id='popupDiv'>";
	echo "<img id='loadingGIF' style='display:inline' src='themes/default/images/img_loading.gif' /><span style='display:inline' id='loadingText'> ".$mod_strings['LBL_REPORT_SENDING_TO_APP']."</span>";
	echo "</div>";
	
}


if (in_array($_REQUEST['return_action'], array("ExportHtml", "ExportPdf", "ExportCsv"))) {
