...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if ($("#" + el.attr("id") + "error").size() <= 0) {
      ...
      err=$(err);
      err.attr("title",errMess).attr("src","modules/asol_Project/resources/jQueryGantt-master/res/alert.gif");
      el.after(err);
    }
  });
  return this;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  bdiv.append("<img src=\"modules/asol_Project/resources/jQueryGantt-master/res/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('modules/asol_Project/resources/jQueryGantt-master/res/black_70.png')"});
  ...
  // ASOL
  var img=$("<img src='modules/asol_Project/resources/jQueryGantt-master/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='"+lang('GANTT_LBL_CLOSE')+"'>");
  // ASOL
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
